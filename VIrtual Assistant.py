...
def ask_from_bot():
    answers_on=['Yes I have turned on the light','Ok I\'ll do it for you!','Sure, on it.','Got you, done!']
    answers_off=['Yes I have turned off the light','Ok I\'ll do it for you!','Sure, on it.','Got you, done!']
    ...
    if 'livingroom' in query or ('living' in query and 'room' in query) or ('hall' in query and 'dining' not in query):
        if any(word in query for word in q_wordList):
            ...
            if 'is' in query or 'are' in query:
                ...
                if retCode==1:
                    answer_from_bot=random.choice(answers_on)
                else:
                ...
                if retCode==1:
                    answer_from_bot=random.choice(answers_off)
                else:
                    answer_from_bot="Could not read from cloud, try again in some time."
            else:
                #error message
    ...
    elif 'fan' in query and 'room' not in query:
        if any(word in query for word in q_wordList):
            if 'on' in query:
                #read status and give message
                stat = read_data_thingspeak(4)
                ...
                #error message
                answer_from_bot=random.choice(errors)
        elif 'is' in query or 'are' in query:
            query_list = [[('is', 'fan'), ('is', 'on')],[('is', 'fan'), ('is', 'off')]]
            c=0
            for q1,q2 in query_list:
                c+=1
